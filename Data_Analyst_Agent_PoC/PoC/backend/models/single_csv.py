...
1. Decide which DataFrame(s) are relevant to answer the query. Use `dfs["file_id"]` to access a DataFrame.
2. If the query requires joining/merging, do so using pandas.
3. Use **pandas** for data manipulation and **plotly.graph_objects (go)** for plotting.
4. Assign the final output to a variable named `result`.
5. Create only **one meaningful plot** with:
    - Proper axis labels
    - Clear and informative title
    - Clean tick formatting (rotate where needed)
6. Axis rules:
    ...
    - Categorical: ensure proper ordering, group rare items into `'Other'` if >10 categories
    - Numeric: ensure readable scale
7. Always validate inputs before using:
    - Check column existence: if not found, raise `ValueError(f"Column '{{col}}' not found")`
    - For `pd.cut()` or histogram bins, ensure bins are sorted and strictly increasing.
    - For division or ratio calculations, check denominator for 0 before dividing.
    - When grouping by categorical variables, always check the number of unique values. If >10, group least frequent into `'Other'`.
8. When filtering or binning:
    - Use defensive coding to avoid empty DataFrames, invalid bin edges, or label mismatches.
9. Do not assume categories, values, or ranges exist unless they are shown above.
10. Clean string values: strip spaces, ensure consistent casing (e.g., title case or lowercase).
11. Do NOT use `.show()` or `.write_image()` or save/display the plot. Just assign to `result`.
12. Always assign the plot as `result = fig` (where `fig` is the `go.Figure()` object).
...
# === API ROUTES ===
#returns code and should plot flag
...
#             temperature=0.2
#         )
#     full_response = response.choices[0].message.content #response.output[-1].content[0].text #response.output[1].content[0].text #response.choices[0].message.content
...
async def CodeGenerationAgent(payload: CodeRequest):
    ...
    for file_id in payload.file_ids:
        ...
        dfs[file_id].columns = preprocess_column_names(dfs[file_id].columns)
...
#     env = {"pd": pd, "df": pd.read_csv(cleaned_csv), "px":px, "go": go, "np": np, "plt": plt, "Figure": Figure, "base64": base64, "io": io}
...
async def ReasoningAgent(payload: ReasoningRequest):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            # Optionally add df_info to the prompt if you want the LLM to know about the files
...
async def clean_data(request: Request):
    try:
        ...
        try:
            ...
            #cleaned_df.to_csv(f"{UPLOAD_DIR}/cleaned_data_"+{file_id}+".csv", index=False)
